...
function generateAlertas({ cotacoes, contratos, spotPrice, histArroba, histMilho }: AlertasFeedProps): Alerta[] {
  ...
  if (spotPrice != null && contratos.length > 2) {
    ...
    if (spread < -8) {
      alertas.push({ timestamp: ts, categoria: "Futuros", texto: `${midContract.codigo} em desconto de R$${Math.abs(spread).toFixed(0)}/@ vs spot — mercado precifica queda`, cor: "var(--brand)" });
    } else if (spread < -3) {
  ...
  if (cotacoes?.cdi_anual != null && spotPrice != null) {
    ...
    alertas.push({ timestamp: ts, categoria: "CDI", texto: `Custo de carrego: R$${custo90d}/@ em 90 dias (CDI ${cdiMensal}% a.m.)`, cor: "var(--text-tertiary)" });
  }
  ...
  return (
    <div className="flex flex-col" style={{ gap: 9 }}>
      {alertas.map((a, i) => (
        ...
        >
          <span className="block" style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: "var(--text-tertiary)", marginBottom: 3 }}>
            {a.timestamp} · {a.categoria}
          </span>
          <span style={{ fontFamily: "Inter, sans-serif", fontSize: 11, color: "var(--text-primary)", lineHeight: 1.5 }}>
            {a.texto}
          </span>
        </div>
      ))}
