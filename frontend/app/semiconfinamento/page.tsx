...
const DEFAULTS: SemiconfinamentoRequest = {
  ...
  dias_ciclo: 110, custo_reposicao_total: 390000,
  custo_arrendamento_dia: 2.0, custo_manutencao_pasto_dia: 0.8,
  consumo_suplemento_kg_dia: 3.5, custo_suplemento_kg: 0.95,
  custo_sanidade_dia: 0.7, custo_mao_obra_dia: 1.1,
  preco_venda: 315, rendimento_carcaca: 0.53,
};
...
export default function SemiconfinamentoPage() {
  ...
  const calculate = useCallback(async (req: SemiconfinamentoRequest) => { setLoading(true); setError(null); try { setData(await calcularSemiconfinamento(req)); } catch (e: unknown) { setError(e instanceof Error ? e.message : "Erro"); } finally { setLoading(false); } }, []);
  ...
  const set = (key: keyof SemiconfinamentoRequest, value: number) => setForm((f) => ({ ...f, [key]: value }));
  const r = data?.resultado;

  return (
    <div className="space-y-8">
      <div>
        <h1 className="font-display text-3xl font-semibold text-t-primary">Semiconfinamento</h1>
        <p className="text-sm text-t-secondary mt-1">Indicador central: custo por arroba · eficiencia da suplementacao</p>
      </div>
      ...
      {r && data && (
        <>
          <Semaforo status={form.preco_venda < r.break_even_price ? "vermelho" : r.margem_percentual < 0.08 ? "amarelo" : "verde"} titulo={form.preco_venda < r.break_even_price ? "Abaixo do break-even" : r.margem_percentual < 0.08 ? "Margem apertada" : "Margem saudavel"} detalhe={`Margem de ${fmtPct(r.margem_percentual)} — spread de ${fmtBRL(form.preco_venda - r.break_even_price)}/@ sobre o break-even`} />
