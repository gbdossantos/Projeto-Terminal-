...
const DEFAULTS: CriaRequest = {
  ...
  peso_desmama_kg: 195, custo_nutricao_ua_ano: 480, custo_sanidade_ua_ano: 120,
  custo_reproducao_ua_ano: 180, custo_mao_obra_ua_ano: 200,
  custo_arrendamento_ua_ano: 350, valor_matriz: 4800, outros_custos_ua_ano: 80,
};
...
export default function CriaPage() {
  ...
  const calculate = useCallback(async (req: CriaRequest) => { setLoading(true); setError(null); try { setData(await calcularCria(req)); } catch (e: unknown) { setError(e instanceof Error ? e.message : "Erro"); } finally { setLoading(false); } }, []);
  ...
  const set = (key: keyof CriaRequest, value: number) => setForm((f) => ({ ...f, [key]: value }));
  const r = data?.resultado;

  return (
    <div className="space-y-8">
      <div>
        <h1 className="font-display text-3xl font-semibold text-t-primary">Sistema de cria</h1>
        <p className="text-sm text-t-secondary mt-1">Indicador central: custo por bezerro produzido</p>
      </div>
